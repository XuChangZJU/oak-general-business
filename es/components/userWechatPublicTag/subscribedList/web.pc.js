...
export default function Render(props) {
    ...
    return (<>
            <FilterPanel entity="wechatUser" oakPath={oakFullpath} columns={[
            {
                attr: 'nickname',
                op: '$includes',
            },
        ]}/>
            <Table loading={oakLoading} dataSource={wechatUsers || []} rowKey="openId" columns={[
            {
                dataIndex: 'openId',
                title: '用户标识',
                width: 150,
            ...
            <Modal title="用户详情" open={open} onCancel={() => {
            setOpen(false);
            ...
            <Modal title="给用户打标签" open={editOpen} onCancel={() => {
            setEditOpen(false);
            setEditTags([]);
            setOpenId('');
                ...
                <Form.Item label="选择标签" required>
                    <Checkbox.Group options={tags} onChange={(values) => {
            setEditTags(values);
        }} value={editTags}/>
                </Form.Item>
            </Modal>
            <Modal open={syncOpen} onCancel={() => {
            setSyncOpen(false);
            setOpenId('');
        }} footer={null} title="同步标签" width={800}>
                <UserWechatPublicTagList oakAutoUnmount={true} oakPath={`$wechatUser-userWechatPublcTag-list-${openId}`} openId={openId} applicationId={applicationId}/>
            </Modal>
        </>);
}
