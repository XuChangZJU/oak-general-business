...
export default function Render(props) {
    ...
    return (<Form colon labelCol={{ span: 4 }} wrapperCol={{ span: 8 }}>
            ...
            <Form.Item label={'输入原密码'} name="prevPassword" help={validateHelp0} rules={[
            {
                required: true,
                message: '请输入原密码',
                validator: (_, value) => {
        ...
        ]} hasFeedback validateStatus={validateStatus0}>
                <Input.Password value={prevPassword} onChange={(e) => {
            const strValue = e.target.value;
            setPrevPassword(strValue);
        }} iconRender={(visible) => (visible ? <EyeTwoTone /> : <EyeInvisibleOutlined />)} placeholder={'请输入原密码'}/>
            </Form.Item>
            <Form.Item label={'输入新密码'} name="password" help={validateHelp1} rules={[
            {
                required: true,
                message: '请输入新密码',
                validator: (_, value) => {
