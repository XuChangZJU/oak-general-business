...
export default function Render(props) {
    ...
    return (<>
            ...
            setOpen(true);
                ...
                {dirtyIds.length > 0 && (<Button type="primary" onClick={() => {
                execute();
            }}>
                        {t('common::action.confirm')}
                    </Button>)}
            </Space>
            <Table loading={oakLoading} dataSource={mtt} rowKey="id" columns={[
            {
                dataIndex: 'type',
                title: '消息类型',
                width: 180,
                ...
                render: (value, record, index) => {
                    if (dirtyIds.includes(record.id) &&
                        ...
                        return (<Select style={{
                                ...
                                templateId: e,
                            }, record.id)} options={smsTemplates.map((ele) => ({
                                value: ele.id,
            ...
            },
            {
                ...
                render: (value, record, index) => {
                    return (<>
                                    {!record.$$deleteAt$$ ? (<Button type="link" danger onClick={() => {
