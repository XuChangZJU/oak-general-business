            ...
            <Tabs tabPosition={'top'} size={'middle'} type="card" items={[
            {
                ...
                children: (<Form colon={true} labelAlign="left" layout="vertical" style={{ marginTop: 10 }}>
                                <Form.Item label="webApiKey">
                                    <>
                                        <Input placeholder="请输入webApiKey" type="text" value={map?.webApiKey} onChange={(e) => setValue(`webApiKey`, e.target.value)}/>
                                    </>
                                </Form.Item>
                            </Form>),
            },
...
export default function Cos(props) {
    ...
    return (<Space direction="vertical" size="middle" style={{ display: 'flex' }}>
            ...
            <Qiniu map={amap} setValue={(path, value) => setValue(`amap.${path}`, value)}/>
        </Space>);
}
