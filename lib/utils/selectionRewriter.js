    ...
    for (const attr in filter) {
        if (attr.startsWith('#') || attr === '$text' || attr.toLowerCase().startsWith(types_1.EXPRESSION_PREFIX)) {
            ...
            if (attr.endsWith('Id') && attr !== 'entityId' && schema[entity].attributes[attr]?.type === 'ref') {
                // 只要是指向user的ref都要处理
                const rel = (0, relation_1.judgeRelation)(schema, entity, attr.slice(0, attr.length - 2));
                if (rel === 'user') {
                    addOrLogic([
                        {
                            [attr]: filter[attr],
                ...
                (0, assert_1.assert)(filter.entityId);
                addOrLogic([
                    {
                ...
                if (rel === 2) {
                    ...
                    const [e] = rel;
                    (0, assert_1.assert)(e !== 'user', '会出现一对多user的情况么');
                    Object.assign(filter2, {
...
function rewriteSelection(schema, entity, selection, context, option, isAggr) {
