                    ...
                    if (qrCodeType) {
                        ...
                        switch (qrCodeType) {
                            ...
                            case 'wechatMpDomainUrl': {
                                if (!(self_1.type === 'wechatMp' &&
                                    self_1.config.qrCodePrefix)) {
                        ...
                        if (sysConfig.App.qrCodeApplicationId) {
                            ...
                            self_2 = applications.find(function (ele) { return ele.id === applicationId; });
                            // 如果本身是服务号，则优先用自己的
                            if (self_2.type === 'wechatPublic' &&
                                self_2.config.isService) {
                                appId = applicationId;
                                appType = 'wechatPublic';
                            }
                            else if ((self_2 === null || self_2 === void 0 ? void 0 : self_2.type) === 'wechatMp') {
                                // 如果本身是小程序，则次优先用小程序的地址码，再次优先用二维码
                                appId = self_2.id;
                                if (self_2.config.qrCodePrefix) {
                                    appType = 'wechatMpDomainUrl';
