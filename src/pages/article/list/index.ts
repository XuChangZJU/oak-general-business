...
export default OakComponent({
  ...
  lifetimes: {
    async ready() {
      ...
      const newArticleMenus = articleMenus?.map((ele) => {
        return {
          ...ele,
          logo: this.features.extraFile.getUrl(ele?.extraFile$entity?.find((ele) => ele.tag1 === 'logo'))
        }
      })
  ...
  methods: {
    getOpenKeys(targetKey: string, treeData: DataNode[], openKeys: string[] = []): string[] {
        ...
        for (const node of nodes) {
          if (node.key === targetKey) {
            ...
            if (isOpen) {
              ...
              openKeys = openKeys.filter((key) => key !== node.key && !key.startsWith(`${node.key}-`));
            } else {
      ...
      const findParentNode = (node: DataNode | any): DataNode | undefined => {
        if (!node.parentKey) {
          return undefined;
        }
        ...
        if(id) {
          ...
          if(articleMenu && articleMenu?.length > 1) {
            this.setState({
              ...
              isChildren: true,
              logo: this.features.extraFile.getUrl(articleMenu[0]?.extraFile$entity?.find((ele) => ele.tag1 === 'logo'))
            })
          }
          if(articleMenu && articleMenu?.length === 1) {
            this.setState({
