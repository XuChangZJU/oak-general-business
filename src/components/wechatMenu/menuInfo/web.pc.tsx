...
export default function Render(
    ...
    const [type, setType] = useState('' as 'image' | 'news' | 'voice' | 'video' | 'text' | '');
    ...
    return (
        <div className={Style.container}>
            {
                config && config.button && config.button.length > 0 && (selectedBtn !== 0 || selectedSubBtn !== 0) ? (
                    <div className={Style.upsertMenu}>
                        <div className={Style.content}>
                            <div className={Style.title}>{selectedSubBtn !== 0 ? '子菜单信息' : '菜单信息'}</div>
                                ...
                                <Form.Item
                                    ...
                                    help={
                                        <div>
                                            ...
                                            {
                                                errorInfo && <div style={{ color: '#fa5151' }}>{errorInfo}</div>
                                            }
                                        </div>}
                                >
                            ...
                            {
                                config.button[currentIndex]?.sub_button?.length === 0 && selectedSubBtn === 0
                                    || selectedSubBtn > 0 ? (
                                    <>
                                        <Form.Item
                                            colon={false}
                                            label={<div className={Style.label}>消息类型</div>}
                                        >
                                            <Radio.Group value={msgType} onChange={(val) => setMsgType(val.target.value)}>
                                                <Radio value={'sendMsg'}>发送消息</Radio>
                                        ...
                                        </Form.Item>
                                        {
                                            msgType === 'sendMsg' ? (
                                                <>
                                                    <Form.Item
                                                        ...
                                                        label={
                                                            <div className={Style.label}>
                                                                {
                                                                    decideMenuContentLabel(decidedMenuContent, type as "video" | "image" | "text" | "news" | "voice")
                                                                }
                                                            </div>}
                                                    >
                                                        {
                                                            !decidedMenuContent && type !== 'text' ? <div className={Style.menuContent}>
                                                                ...
                                                                : type === 'image' ?
                                                                    <div className={Style.coverImage}>
                                                                        <img className={Style.img} src={decidedMenuContent.url} />
                                                                    ...
                                                                    : type === 'voice' ?
                                                                        <div className={Style.fileCover}>
                                                                            <a href={decidedMenuContent.url} download={true} style={{ color: '#1677FF', cursor: 'pointer' }}>
                                                                                <DownloadOutlined />{decidedMenuContent.media_id}
                                                                            </a>
                                                                            <div className={Style.buttonGroup}>
                                                                                <div className={Style.buttonItem} onClick={() => {
                                                                                    const modal = confirm({
                                                                        ...
                                                                        : type === 'video' ?
                                                                            ...
                                                                            : type === 'news' ?
                                                                                <div className={Style.news}>
                                                                                    <ShowNews
                                                                                        news={decidedMenuContent?.content?.news_item}
                                                                                        oakAutoUnmount={false}
                                                                                    />
                                                                                    <div className={Style.buttonGroup} style={{ height: '100%' }}>
                                                                                        <div className={Style.buttonItem} onClick={() => {
                                                                                            const modal = confirm({
                                                                                                title: '确定删除该图文信息吗？',
                                                                                                content: '删除后不可恢复',
                                                        ...
                                                        }
                                                        {
                                                            ...
                                                            <div className={Style.editor}>
                                                                <TextClick
                                                                    oakAutoUnmount={true}
                                                                    value={decidedMenuContent}
                                                                    getDecidedMenuContent={getDecidedMenuContent}
                                                                />
                                                                <div className={Style.buttonGroup} style={{ height: 36, position: 'absolute', right: -50 }}>
                                                                    <div className={Style.buttonItem} onClick={() => {
                                                                        const modal = confirm({
                                                                            title: '确定删除该文字吗？',
                                                                            content: '删除后不可恢复',
                                                    ...
                                                    >
                                                        <WechatMaterialLibrary
                                                            oakAutoUnmount={true}
                                                            type={type as 'news' | 'image' | 'video' | 'voice'}
                                                ...
                                                <Form.Item
                                                    colon={false}
                                                    label={<div className={Style.label}>网页链接</div>}
                                                    help={
                                                        <>{errorUrlInfo && <div style={{ color: '#fa5151' }}>{errorUrlInfo}</div>}</>
                                                    }
                                                >
                                                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                                                        <Input placeholder='公众号链接:请以https://或http://开头'
                                                            style={{ width: 340 }}
                                                            value={url}
                                                            onChange={(val) => {
                                                        ...
                                                        />
                                                        <a style={{ padding: '10px 0' }} onClick={() => { setOpen(true); getOpen(true) }}>
                                                            选择图文链接
                                                        </a>
                                                    </div>
                                                    <Modal
                                                        ...
                                                        footer={null}
                                                        title={'选择图文链接'}
                                                        onCancel={() => { setOpen(false); getOpen(false) }}
                                                        width={600}
                                                    >
                                                        <SelectArticle
                                                            oakAutoUnmount={true}
                                                ...
                                                <Form.Item
                                                    colon={false}
                                                    label={<div className={Style.label}>小程序</div>}
                                                >
                                                    <Button onClick={() => {
                                                        setOpen(true);
                                                        getOpen(true);
