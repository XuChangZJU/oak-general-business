    ...
    if (oakFullpath) {
        return (
            <>
                ...
                >
                    <DomainUpsertItem
                        data={list.find(ele => ele.id === upsertId) as EntityDict['domain']['OpSchema']}
                        update={(attr, value) => updateItem({
                            [attr]: value,
                        }, upsertId)}
                    />
                </Modal>
                        ...
                        },
                        {
                            ...
                            render: (value, record, index) => {
                                return (
                                    <Row>
                                        {
                                            record['#oakLegalActions']?.includes('update') && <Button
                                                type='link'
                                                onClick={() => setUpsertId(record.id!)}
                                            >
                                                {t('common::action.update')}
                                            </Button>
                                        }
                                        {
                                            record['#oakLegalActions']?.includes('remove') && <Button
                                                type='link'
                                                onClick={() => setRemoveId(record.id!)}
                                            >
